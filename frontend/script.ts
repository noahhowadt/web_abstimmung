...
function getAppointments() {
  $.get("../backend/get-appointments.php", (data) => {
    ...
    for (const appointment of JSON.parse(data)) {
      ...
      if (appointment.expires_at < new Date()) {
        // Wenn das Ablaufdatum abgelaufen ist, entferne das Klickereignis für den "Details"-Button
        appointmentElement.find(".details").off("click");
      }
